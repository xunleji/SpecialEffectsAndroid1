...
/**
 * 未读消息提示View,显示小红点或者带有数字的红点: 数字一位,圆 数字两位,圆角矩形,圆角是高度的一半 数字超过两位,显示99+
 */
public class UnreadMsgUtils {
	...
	public static void show(RoundTextView rtv, int num) {
		...
		RelativeLayout.LayoutParams lp = (RelativeLayout.LayoutParams) rtv
				.getLayoutParams();
		...
		if (num <= 0) {// 圆点,设置默认宽高
			...
			if (num > 0 && num < 10) {// 圆
				...
				rtv.setPadding((int) (6 * dm.density), 0,
						(int) (6 * dm.density), 0);
				rtv.setText(num + "");
			} else {// 数字超过两位,显示99+
				lp.width = RelativeLayout.LayoutParams.WRAP_CONTENT;

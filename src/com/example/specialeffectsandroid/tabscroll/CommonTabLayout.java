...
public class CommonTabLayout extends FrameLayout implements
	...
	public CommonTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		// get layout_height
		String height = attrs.getAttributeValue(
	...
	private void obtainAttributes(Context context, AttributeSet attrs) {
		...
		indicatorStyle = ta.getInt(
				R.styleable.CommonTabLayout_tl_indicator_style, 0);
		indicatorColor = ta.getColor(
				R.styleable.CommonTabLayout_tl_indicator_color, Color
						.parseColor(indicatorStyle == STYLE_BLOCK ? "#4B6A87"
								: "#ffffff"));
		indicatorHeight = ta.getDimension(
				R.styleable.CommonTabLayout_tl_indicator_height,
				dp2px(indicatorStyle == STYLE_TRIANGLE ? 4
						: (indicatorStyle == STYLE_BLOCK ? -1 : 2)));
		...
		indicatorMarginLeft = ta.getDimension(
				R.styleable.CommonTabLayout_tl_indicator_margin_left, dp2px(0));
		...
		indicatorMarginBottom = ta.getDimension(
				...
				dp2px(indicatorStyle == STYLE_BLOCK ? 7 : 0));
		indicatorAnimEnable = ta.getBoolean(
				R.styleable.CommonTabLayout_tl_indicator_anim_enable, true);
		indicatorBounceEnable = ta.getBoolean(
				R.styleable.CommonTabLayout_tl_indicator_bounce_enable, true);
		indicatorAnimDuration = ta.getInt(
				R.styleable.CommonTabLayout_tl_indicator_anim_duration, -1);
		indicatorGravity = ta.getInt(
				R.styleable.CommonTabLayout_tl_indicator_gravity,
				Gravity.BOTTOM);

		underlineColor = ta.getColor(
				R.styleable.CommonTabLayout_tl_underline_color,
				Color.parseColor("#ffffff"));
		underlineHeight = ta.getDimension(
				R.styleable.CommonTabLayout_tl_underline_height, dp2px(0));
		underlineGravity = ta.getInt(
				R.styleable.CommonTabLayout_tl_underline_gravity,
				Gravity.BOTTOM);

		dividerColor = ta.getColor(
				R.styleable.CommonTabLayout_tl_divider_color,
				Color.parseColor("#ffffff"));
		dividerWidth = ta.getDimension(
				R.styleable.CommonTabLayout_tl_divider_width, dp2px(0));
		dividerPadding = ta.getDimension(
				R.styleable.CommonTabLayout_tl_divider_padding, dp2px(12));

		textsize = ta.getDimension(R.styleable.CommonTabLayout_tl_textsize,
				sp2px(13f));
		textSelectColor = ta.getColor(
				R.styleable.CommonTabLayout_tl_textSelectColor,
				Color.parseColor("#ffffff"));
		textUnselectColor = ta.getColor(
				R.styleable.CommonTabLayout_tl_textUnselectColor,
				Color.parseColor("#AAffffff"));
		textBold = ta
				.getBoolean(R.styleable.CommonTabLayout_tl_textBold, false);
		textAllCaps = ta.getBoolean(R.styleable.CommonTabLayout_tl_textAllCaps,
				false);

		iconVisible = ta.getBoolean(R.styleable.CommonTabLayout_tl_iconVisible,
				true);
		iconGravity = ta.getInt(R.styleable.CommonTabLayout_tl_iconGravity,
				Gravity.TOP);
		iconWidth = ta.getDimension(R.styleable.CommonTabLayout_tl_iconWidth,
				dp2px(0));
		iconHeight = ta.getDimension(R.styleable.CommonTabLayout_tl_iconHeight,
				dp2px(0));
		iconMargin = ta.getDimension(R.styleable.CommonTabLayout_tl_iconMargin,
				dp2px(2.5f));

		tabSpaceEqual = ta.getBoolean(
				R.styleable.CommonTabLayout_tl_tab_space_equal, true);
		tabWidth = ta.getDimension(R.styleable.CommonTabLayout_tl_tab_width,
				dp2px(-1));
		tabPadding = ta.getDimension(
				R.styleable.CommonTabLayout_tl_tab_padding, tabSpaceEqual
						|| tabWidth > 0 ? dp2px(0) : dp2px(10));
	...
	/** 关联数据支持同时切换fragments */
	public void setTabData(ArrayList<CustomTabEntity> tabEntitys,
			FragmentManager fm, int containerViewId,
			ArrayList<Fragment> fragments) {
		fragmentChangeManager = new FragmentChangeManager(fm, containerViewId,
				fragments);
	...
	public void notifyDataSetChanged() {
		...
		this.tabCount = tabEntitys.size();
		View tabView;
		for (int i = 0; i < tabCount; i++) {
			if (iconGravity == Gravity.LEFT) {
				tabView = View.inflate(context, R.layout.layout_tab_left, null);
			} else if (iconGravity == Gravity.RIGHT) {
				tabView = View
						.inflate(context, R.layout.layout_tab_right, null);
			} else if (iconGravity == Gravity.BOTTOM) {
				tabView = View.inflate(context, R.layout.layout_tab_bottom,
						null);
			} else {
				tabView = View.inflate(context, R.layout.layout_tab_top, null);
			}
	...
	private void addTab(final int position, View tabView) {
		...
		tv_tab_title.setText(tabEntitys.get(position).getTabTitle());
		ImageView iv_tab_icon = (ImageView) tabView
				.findViewById(R.id.iv_tab_icon);
		iv_tab_icon.setImageResource(tabEntitys.get(position)
		...
		tabView.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				Diary.print("currentTab=" + currentTab + "lastTab=" + lastTab);
		...
		/** 每一个Tab的布局参数 */
		LinearLayout.LayoutParams lp_tab = tabSpaceEqual ? new LinearLayout.LayoutParams(
				0, LayoutParams.MATCH_PARENT, 1.0f)
				: new LinearLayout.LayoutParams(LayoutParams.WRAP_CONTENT,
						LayoutParams.MATCH_PARENT);
		if (tabWidth > 0) {
			lp_tab = new LinearLayout.LayoutParams((int) tabWidth,
					LayoutParams.MATCH_PARENT);
		}
	...
	private void updateTabStyles() {
		for (int i = 0; i < tabCount; i++) {
			...
			TextView tv_tab_title = (TextView) tabView
					.findViewById(R.id.tv_tab_title);
			tv_tab_title.setTextColor(i == currentTab ? textSelectColor
					: textUnselectColor);
			tv_tab_title.setTextSize(TypedValue.COMPLEX_UNIT_PX, textsize);
			// tv_tab_title.setPadding((int) tabPadding, 0, (int) tabPadding,
			// 0);
			if (textAllCaps) {
				tv_tab_title.setText(tv_tab_title.getText().toString()
						.toUpperCase());
			}
			...
			if (iconVisible) {
				...
				CustomTabEntity tabEntity = tabEntitys.get(i);
				iv_tab_icon.setImageResource(i == currentTab ? tabEntity
						.getTabSelectedIcon() : tabEntity
						.getTabUnselectedIcon());
				LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(
						iconWidth <= 0 ? LinearLayout.LayoutParams.WRAP_CONTENT
								: (int) iconWidth,
						iconHeight <= 0 ? LinearLayout.LayoutParams.WRAP_CONTENT
								: (int) iconHeight);
				if (iconGravity == Gravity.LEFT) {
	...
	private void updateTabSelection(int position) {
		for (int i = 0; i < tabCount; ++i) {
			...
			TextView tab_title = (TextView) tabView
					.findViewById(R.id.tv_tab_title);
			tab_title.setTextColor(isSelect ? textSelectColor
					: textUnselectColor);
			ImageView iv_tab_icon = (ImageView) tabView
					.findViewById(R.id.iv_tab_icon);
			CustomTabEntity tabEntity = tabEntitys.get(i);
			iv_tab_icon.setImageResource(isSelect ? tabEntity
					.getTabSelectedIcon() : tabEntity.getTabUnselectedIcon());
		}
	}
	...
	private void calcIndicatorRect() {
		...
		if (indicatorWidth < 0) { // indicatorWidth小于0时,原jpardogo's
									// PagerSlidingTabStrip

		} else {// indicatorWidth大于0时,圆角矩形以及三角形
			float indicatorLeft = currentTabView.getLeft()
					+ (currentTabView.getWidth() - indicatorWidth) / 2;
	...
	protected void onDraw(Canvas canvas) {
		...
		if (underlineHeight > 0) {
			...
			if (underlineGravity == Gravity.BOTTOM) {
				canvas.drawRect(paddingLeft, height - underlineHeight,
						tabsContainer.getWidth() + paddingLeft, height,
						rectPaint);
			} else {
				canvas.drawRect(paddingLeft, 0, tabsContainer.getWidth()
						+ paddingLeft, underlineHeight, rectPaint);
			}
		}
		...
		if (indicatorStyle == STYLE_TRIANGLE) {
			if (indicatorHeight > 0) {
				...
				trianglePath.lineTo(paddingLeft + indicatorRect.left / 2
						+ indicatorRect.right / 2, height - indicatorHeight);
			...
			if (indicatorHeight < 0) {
				indicatorHeight = height - indicatorMarginTop
			...
			if (indicatorHeight > 0) {
				...
				indicatorDrawable.setBounds(paddingLeft
						+ (int) indicatorMarginLeft + indicatorRect.left,
						(int) indicatorMarginTop, (int) (paddingLeft
								+ indicatorRect.right - indicatorMarginRight),
			...
			/*
			 * rectPaint.setColor(indicatorColor); calcIndicatorRect();
			 * canvas.drawRect(getPaddingLeft() + indicatorRect.left,
			 * getHeight() - indicatorHeight, indicatorRect.right +
			 * getPaddingLeft(), getHeight(), rectPaint);
			 */
				...
				if (indicatorGravity == Gravity.BOTTOM) {
					indicatorDrawable.setBounds(paddingLeft
							...
							height - (int) indicatorHeight
									- (int) indicatorMarginBottom, paddingLeft
									+ indicatorRect.right
									- (int) indicatorMarginRight, height
									- (int) indicatorMarginBottom);
				} else {
					indicatorDrawable.setBounds(paddingLeft
							+ (int) indicatorMarginLeft + indicatorRect.left,
							(int) indicatorMarginTop, paddingLeft
									+ indicatorRect.right
	...
	public void setCurrentTab(int currentTab) {
		if (listener != null) {
			listener.onTabSelect(currentTab);
		}
	...
	public void setIndicatorMargin(float indicatorMarginLeft,
			float indicatorMarginTop, float indicatorMarginRight,
	...
	public void setIconWidth(float iconWidth) {
		this.iconWidth = dp2px(iconWidth);
		updateTabStyles();
	}
	...
	public void setIconMargin(float iconMargin) {
		this.iconMargin = dp2px(iconMargin);
		updateTabStyles();
	}
	...
	private SparseArray<Boolean> initSetMap = new SparseArray<Boolean>();
	...
	public void showMsg(int position, int num) {
		...
		RoundTextView tipView = (RoundTextView) tabView
				.findViewById(R.id.rtv_msg_tip);
		if (tipView != null) {
			...
			if (!iconVisible) {
				...
				setMsgMargin(position, 0, iconGravity == Gravity.LEFT
						|| iconGravity == Gravity.RIGHT ? 4 : 0);
			}
	...
	/**
	 * 设置提示红点偏移,注意 1.控件为固定高度:参照点为tab内容的右上角
	 * 2.控件高度不固定(WRAP_CONTENT):参照点为tab内容的右上角,
	 * 此时高度已是红点的最高显示范围,所以这时bottomPadding其实就是topPadding
	 */
	public void setMsgMargin(int position, float leftPadding,
			float bottomPadding) {
		if (position >= tabCount) {
			position = tabCount - 1;
		}
		...
		if (tipView != null) {
			...
			float textWidth = textPaint.measureText(tv_tab_title.getText()
					.toString());
			float textHeight = textPaint.descent() - textPaint.ascent();
			MarginLayoutParams lp = (MarginLayoutParams) tipView
					.getLayoutParams();

			float iconH = iconHeight;
			float margin = 0;
			if (iconVisible) {
				if (iconH <= 0) {
					iconH = context
			...
			if (iconGravity == Gravity.TOP || iconGravity == Gravity.BOTTOM) {
				...
				lp.topMargin = h > 0 ? (int) (h - textHeight - iconH - margin)
						/ 2 - dp2px(bottomPadding) : dp2px(bottomPadding);
			} else {
				lp.leftMargin = dp2px(leftPadding);
				lp.topMargin = h > 0 ? (int) (h - Math.max(textHeight, iconH))
						/ 2 - dp2px(bottomPadding) : dp2px(bottomPadding);
			}
	...
	class PointEvaluator implements TypeEvaluator<IndicatorPoint> {
		@Override
		public IndicatorPoint evaluate(float fraction,
				IndicatorPoint startValue, IndicatorPoint endValue) {
			float left = startValue.left + fraction
					* (endValue.left - startValue.left);
			float right = startValue.right + fraction

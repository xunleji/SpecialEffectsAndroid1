...
public class SlidingTabLayout extends HorizontalScrollView implements
	...
	private void obtainAttributes(Context context, AttributeSet attrs) {
		...
		indicatorStyle = ta.getInt(
				R.styleable.SlidingTabLayout_tl_indicator_style, STYLE_NORMAL);
		indicatorColor = ta.getColor(
				R.styleable.SlidingTabLayout_tl_indicator_color, Color
		...
		indicatorMarginLeft = ta
				.getDimension(
						R.styleable.SlidingTabLayout_tl_indicator_margin_left,
						dp2px(0));
		...
		indicatorGravity = ta.getInt(
				R.styleable.SlidingTabLayout_tl_indicator_gravity,
				Gravity.BOTTOM);
		indicatorWidthEqualTitle = ta.getBoolean(
				R.styleable.SlidingTabLayout_tl_indicator_width_equal_title,
				false);

		underlineColor = ta.getColor(
				R.styleable.SlidingTabLayout_tl_underline_color,
				Color.parseColor("#ffffff"));
		underlineHeight = ta.getDimension(
				R.styleable.SlidingTabLayout_tl_underline_height, dp2px(0));
		underlineGravity = ta.getInt(
				R.styleable.SlidingTabLayout_tl_underline_gravity,
				Gravity.BOTTOM);

		dividerColor = ta.getColor(
				R.styleable.SlidingTabLayout_tl_divider_color,
				Color.parseColor("#ffffff"));
		dividerWidth = ta.getDimension(
				R.styleable.SlidingTabLayout_tl_divider_width, dp2px(0));
		dividerPadding = ta.getDimension(
				R.styleable.SlidingTabLayout_tl_divider_padding, dp2px(12));

		textsize = ta.getDimension(R.styleable.SlidingTabLayout_tl_textsize,
				sp2px(14));
		textSelectColor = ta.getColor(
				R.styleable.SlidingTabLayout_tl_textSelectColor,
				Color.parseColor("#ffffff"));
		textUnselectColor = ta.getColor(
				R.styleable.SlidingTabLayout_tl_textUnselectColor,
				Color.parseColor("#AAffffff"));
		textBold = ta.getBoolean(R.styleable.SlidingTabLayout_tl_textBold,
				false);
		textAllCaps = ta.getBoolean(
				R.styleable.SlidingTabLayout_tl_textAllCaps, false);

		tabSpaceEqual = ta.getBoolean(
				R.styleable.SlidingTabLayout_tl_tab_space_equal, false);
		tabWidth = ta.getDimension(R.styleable.SlidingTabLayout_tl_tab_width,
				dp2px(-1));
		tabPadding = ta.getDimension(
				R.styleable.SlidingTabLayout_tl_tab_padding, tabSpaceEqual
						|| tabWidth > 0 ? dp2px(0) : dp2px(20));
	...
	public void setViewPager(ViewPager vp, String[] titles,
			FragmentActivity fa, ArrayList<Fragment> fragments) {
		if (vp == null) {
		...
		this.vp = vp;
		this.vp.setAdapter(new InnerPagerAdapter(
				fa.getSupportFragmentManager(), fragments, titles));
	...
	public void notifyDataSetChanged() {
		...
		this.tabCount = titles == null ? vp.getAdapter().getCount()
				: titles.length;
		View tabView;
		for (int i = 0; i < tabCount; i++) {
			if (vp.getAdapter() instanceof CustomTabProvider) {
				tabView = ((CustomTabProvider) vp.getAdapter())
						.getCustomTabView(this, i);
			} else {
				tabView = View.inflate(context, R.layout.layout_tab, null);
			}

			CharSequence pageTitle = titles == null ? vp.getAdapter()
					.getPageTitle(i) : titles[i];
	...
	private void addTab(final int position, String title, View tabView) {
		...
		tabView.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				Diary.print("getCurrentItem=" + vp.getCurrentItem()
						+ "position=" + position);
	...
	private void updateTabStyles() {
		for (int i = 0; i < tabCount; i++) {
			...
			// v.setPadding((int) tabPadding, v.getPaddingTop(), (int)
			// tabPadding, v.getPaddingBottom());
	...
	public void onPageScrolled(int position, float positionOffset,
		...
		/**
		 * position:当前View的位置 currentPositionOffset:当前View的偏移量比例.[0,1)
		 */
	...
	private void scrollToCurrentTab() {
		...
		int offset = (int) (currentPositionOffset * tabsContainer.getChildAt(
				currentTab).getWidth());
		/** 当前Tab的left+当前Tab的Width乘以positionOffset */
		int newScrollX = tabsContainer.getChildAt(currentTab).getLeft()
				+ offset;
		...
		if (newScrollX != lastScrollX) {
			...
			/**
			 * scrollTo（int x,int y）:x,y代表的不是坐标点,而是偏移量 x:表示离起始位置的x水平方向的偏移量
			 * y:表示离起始位置的y垂直方向的偏移量
			 */
	...
	private void calcIndicatorRect() {
		...
		if (indicatorStyle == STYLE_NORMAL && indicatorWidthEqualTitle) {
			...
			float textWidth = textPaint.measureText(tab_title.getText()
					.toString());
		...
		if (this.currentTab < tabCount - 1) {
			...
			if (indicatorStyle == STYLE_NORMAL && indicatorWidthEqualTitle) {
				TextView next_tab_title = (TextView) nextTabView
						.findViewById(R.id.tv_tab_title);
				textPaint.setTextSize(textsize);
				float nextTextWidth = textPaint.measureText(next_tab_title
						.getText().toString());
		...
		if (indicatorWidth < 0) { // indicatorWidth小于0时,原jpardogo's
			...
			if (this.currentTab < tabCount - 1) {
				...
				indicatorLeft = indicatorLeft
						...
						* (currentTabView.getWidth() / 2 + nextTab.getWidth() / 2);
			}
	...
	public void setMsgMargin(int position, float leftPadding,
		...
		if (tipView != null) {
			...
			MarginLayoutParams lp = (MarginLayoutParams) tipView
					.getLayoutParams();
			lp.leftMargin = tabWidth >= 0 ? (int) (tabWidth / 2 + textWidth / 2 + dp2px(leftPadding))
					: (int) (tabPadding + textWidth + dp2px(leftPadding));
			lp.topMargin = h > 0 ? (int) (h - textHeight) / 2
					- dp2px(bottomPadding) : 0;

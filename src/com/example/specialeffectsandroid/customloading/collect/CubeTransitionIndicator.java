...
public class CubeTransitionIndicator extends BaseIndicatorController {
	...
	public void draw(Canvas canvas, Paint paint) {
		...
		for (int i = 0; i < 2; i++) {
			...
			RectF rectF = new RectF(-rWidth / 2, -rHeight / 2, rWidth / 2,
					rHeight / 2);
	...
	public List<Animator> createAnimation() {
		...
		for (int i = 0; i < 2; i++) {
			...
			translationXAnim
					.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
						...
						public void onAnimationUpdate(ValueAnimator animation) {
							translateX[index] = (Float) animation
									.getAnimatedValue();
							postInvalidate();
						}
					});
			...
			ValueAnimator translationYAnim = ValueAnimator.ofFloat(startY,
					startY, getHeight() - startY, getHeight() - startY, startY);
			if (i == 1) {
				translationYAnim = ValueAnimator.ofFloat(getHeight() - startY,
						getHeight() - startY, startY, startY, getHeight()
								- startY);
			}

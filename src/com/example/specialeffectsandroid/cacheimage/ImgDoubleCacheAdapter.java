...
public class ImgDoubleCacheAdapter extends BaseAdapter {

    /** listview滚屏是否滚动中 */
    private boolean mBusy = false;

    /** 图片异步加载器 */
    private ImageAsynLoader mImageLoader;
    ...
    public ImgDoubleCacheAdapter(int count, Context context) {
        // 初始化模拟数据
        initData();
        this.mCount = count;
        this.mContext = context;
        mImageLoader = new ImageAsynLoader();
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            convertView = LayoutInflater.
                    from(mContext).inflate(R.layout.double_cache_list_item, null);
        }
        mTextView = (TextView) convertView.findViewById(R.id.tv_tips);
        mImageView = (ImageView) convertView.findViewById(R.id.iv_image);
        mTextView.setText("图片" + position);

        String url = this.URLS[position % URLS.length];
        mImageLoader.loadImage(url, this, mBusy, mImageView);
        return convertView;
    }
